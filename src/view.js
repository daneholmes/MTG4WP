document.addEventListener('DOMContentLoaded', () => {
	const defaultImageElement = document.getElementById('mtg-tools-default-image');
			...
			didRender: () => {
				if (card.backImage) {
					document.getElementById('flip-card-button').addEventListener('click', () => {
						...
						showPopup(card);  // Recursively show the popup with the flipped image
					});
				}
				if (card.foil) {
	...
	document.querySelectorAll('.mtg-tools-card').forEach((cardElement) => {
		const card = {
